...
export default function Circuits3DPage() {
    ...
    const selectedCircuit = circuits.find((circuit) => circuit.id === selectedCircuitId)!;
    const selectedDriver = drivers2025.find((driver) => driver.id === selectedDriverId)!;
    ...
    const handleProgressChange = useCallback(
        (progress: number) => {
            const circuit = circuits.find((item) => item.id === selectedCircuitId)!;
            const telemetryIndex = Math.floor(progress * circuit.telemetry.length) % circuit.telemetry.length;
            setCurrentTelemetry(circuit.telemetry[telemetryIndex]);

            const sectorIndex = circuit.sectors.findIndex(
                (sector) => progress >= sector.startPercent && progress < sector.endPercent
            );

            if (sectorIndex >= 0 && sectorIndex !== lastSectorRef.current) {
                if (sectorIndex === 0 && lastSectorRef.current === circuit.sectors.length - 1) {
                    setSectorTimes((previous) => {
                ...
                } else if (sectorIndex > lastSectorRef.current) {
                    setSectorTimes((previous) => {
                        const updated = [...previous];
                        updated[lastSectorRef.current] = circuit.sectors[lastSectorRef.current].bestTime;
                        return updated;
            ...
            <div className="max-w-7xl mx-auto">
                ...
                >
                    ...
                    <p className="text-f1-text-muted text-lg max-w-2xl">
                        Follow a full lap on a perspective 3D track render with camera modes, live telemetry,
                        and sector timing.
                    </p>
                </motion.div>
